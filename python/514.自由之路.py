...
class Solution:
    def findRotateSteps(self, ring: str, key: str) -> int:
        ...
        for offset, c in enumerate(ring):
            if c not in d:
                d[c] = []
            d[c].append(offset)
        ...
        def dp(offset, d_offset):
            if offset == len(key):
                return 0
            if (offset, d_offset) in mem:
                return mem[(offset, d_offset)]
            next_c = key[offset]
            ret = float('INF')
            for i in d[next_c]:
                ret = min(ret, 1 + dp(offset + 1, i) +
                          min(abs(d_offset - i), len(ring) + i-d_offset, len(ring) - i+d_offset))
            mem[(offset, d_offset)] = ret
            return ret
        return dp(0, 0)
# @lc code=end
