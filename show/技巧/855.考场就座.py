...
class ExamRoom:
    ...
    def seat(self) -> int:
        ...
        self.seat_list.sort()
        l = list(self.seat_list)
        l.append(- self.seat_list[0])
        l.append(2 * (self.n-1) - self.seat_list[-1])
        l.sort()
        ...
        for n in range(len(l)):
            ...
            if max_len > res[1]:
                res = (l[n-1]+max_len, max_len)

        self.seat_list.append(res[0])

        return res[0]

    def leave(self, p: int) -> None:
        self.seat_list.pop(self.seat_list.index(p))
